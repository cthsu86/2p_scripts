...
close all;
beh_rootdir = 'C:\Users\Windows 10\Dropbox\Sehgal Lab\In Vivo Imaging\Raw Data\84C10Gal4_UASGCaMP_RFP_180607';
xlsname = '84C10Gal4_UASGCaMP_RFP_180607.xlsx';
annot_rootdir = beh_rootdir;
vidRootName = 'fc2_save_2018-06-07-'; %Assume that the timestamp of the video will be right after "vidRootName";
fps = 30;
laserOnsetColNum = 3;
laserOffsetColNum = 4;
...
originalNumPxThresh = numPxThresh;
maxNumPxThresh = 10000;
numNoMovementFramesPerThresh = NaN(maxNumPxThresh-originalNumPxThresh+1,2);
for(numPxThresh = originalNumPxThresh:maxNumPxThresh),
    movement2test = interp_movement;
    movementBinary = movement2test>numPxThresh;
    ...
    for(mi = 1:numel(movementDurations)),
        if(movementDurations(mi)<laserDurationThresh_frames),
            movement2test(movementOnsets(mi):movementOffsets(mi)) = 0;
        end;
    end;
    belowThreshIndices = find(movement2test<numPxThresh);
    movement2test(belowThreshIndices) = 0;
    numNoMovementFramesPerThresh(numPxThresh+1,1) = numPxThresh;
    numNoMovementFramesPerThresh(numPxThresh+1,2) = sum(movement2test==0);
    
end;
plot(numNoMovementFramesPerThresh(:,1),numNoMovementFramesPerThresh(:,2),'ko','MarkerSize',3);
% figure;
% % subplot(1,2,1);
