...
TImgRoot = 'TSeries-05092018-1612-230_Cycle00001_Ch2_'; %000001.ome
...
if(saveBlackAndWhite),
    vidObj = VideoWriter([TImgRoot '_BW_upperStretchLim ' num2str(upperStretchLim) '_medfilt'  num2str(medfiltSize) '_medfiltFirstMeanZ.avi']);
else,
...
for(ti = 1:maxFrame2read),
    ...
    pixelDat(:,ti) = double(medFiltFrame(:));
%     useMedian = 0;
...
%         maxZProj = max(pixelDat(:,ti-slicesPerStack+1:ti),[],2);
        maxZProjFrame = reshape(maxZProj,size(medFiltFrame,1),size(medFiltFrame,2));
        maxPixValue = max(double(rawImg(:)));
        perFrameStretchLim = stretchThreshold/maxPixValue;
        if(saveBlackAndWhite),
            %         thisFrame = uint8(imadjust(rawImg,[0 perFrameStretchLim]));
            % perFrameStretchLim
            adjustedFrame = imadjust(uint8(maxZProjFrame),[0 perFrameStretchLim]);
